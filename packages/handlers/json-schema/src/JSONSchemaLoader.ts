import { AggregateError, BaseLoaderOptions, inspect, Loader, Source } from '@graphql-tools/utils';
...
export class JSONSchemaLoader implements Loader<JSONSchemaLoaderOptions> {
  async load(pointer: 'jsonSchema', options: JSONSchemaLoaderOptions): Promise<Source[]> {
    ...
    const finalJSONSchema = options.finalSchema || this.buildFinalJSONSchema(options);
    ...
    this.addExecutionLogicToComposer(schemaComposer, options);

    return [
      {
        schema: schemaComposer.buildSchema(),
        location: `${options.name || pointer}`,
      },
    ];
  }

  async addExecutionLogicToComposer(schemaComposer: SchemaComposer, options: JSONSchemaLoaderOptions) {
    ...
    for (const operationConfig of options.operations) {
      const { httpMethod, rootTypeName, fieldName } = this.getOperationMetadata(operationConfig);
      ...
      if (this.isPubSubOperationConfig(operationConfig)) {
  ...
  private isPubSubOperationConfig(
  ...
  private getOperationMetadata(operationConfig: JSONSchemaOperationConfig) {
  ...
  async buildFinalJSONSchema(options: JSONSchemaLoaderOptions) {
    ...
    for (const operationConfig of options.operations) {
      const { operationType, rootTypeName, fieldName } = this.getOperationMetadata(operationConfig);
